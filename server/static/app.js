const state = {
  ...
  accent: localStorage.getItem('newsai_accent') || 'en-US',
  emotion: parseInt(localStorage.getItem('newsai_emotion') || '40', 10),
  refAudioUrl: '',
  preview: [],
  feedbackEvents: [],
};
...
function setAccent(accent) {
  localStorage.setItem('newsai_accent', accent);
  state.accent = accent;
}

function setEmotion(val) {
  const v = Math.max(0, Math.min(100, parseInt(val || '40', 10)));
  localStorage.setItem('newsai_emotion', String(v));
  state.emotion = v;
}
...
function renderFeed(items) {
  ...
  itemsToShow.forEach((it) => {
    ...
    const ttsBtn = mkBtn('TTS', () => generateTTSForItem(it));
    const avatarBtn = mkBtn('Avatar', () => generateAvatarForItem(it));
    actions.append(likeBtn, dislikeBtn, approveBtn, flagBtn, ttsBtn, avatarBtn);
...
async function generateTTSForItem(item) {
  updateStages('voice', 'active');
  status('Voicing item…');
  try {
    const narration = (item?.variants?.narration) || item?.body || item?.title || '';
    const payload = {
      title: item?.title || '(Untitled)',
      narration,
      lang: item?.lang || state.lang || 'en',
      accent: state.accent,
    ...
    const json = await apiPost('/api/agents/voice/generate_item', payload, 30000);
    updateStages('voice', 'done');
    const it = json?.item || (Array.isArray(json?.items) ? json.items[0] : null);
    const url = it?.audio_url || '(no url)';
    status(`TTS ready • ${url}`);
    pushEvent({ kind: 'info', message: `TTS generated: ${url}` });
  } catch (err) {
    status(`TTS failed: ${String(err.message || err)}`);
  }
}

async function generateAvatarForItem(item) {
  ...
  try {
    ...
    const payload = {
      ...
      accent: state.accent,
      emotion: state.emotion,
      audio_url: (state.refAudioUrl || '').trim() || undefined,
      voice: 'en-US-Neural-1',
      style: 'news-anchor',
      category: 'general',
    };
    const json = await apiPost('/api/agents/avatar/render_item', payload, 60000);
    updateStages('avatar', 'done');
    const it = json?.item || (Array.isArray(json?.items) ? json.items[0] : null);
    const vurl = it?.video_url || '(no url)';
    status(`Avatar ready • ${vurl}`);
    pushEvent({ kind: 'info', message: `Avatar rendered: ${vurl}` });
  } catch (err) {
    status(`Avatar failed: ${String(err.message || err)}`);
  }
}
...
function init() {
  ...
  document.getElementById('accentSelect').value = state.accent;
  document.getElementById('accentSelect').addEventListener('change', (e) => setAccent(e.target.value));
  document.getElementById('emotionRange').value = String(state.emotion);
  document.getElementById('emotionRange').addEventListener('input', (e) => setEmotion(e.target.value));
  ...
  document.getElementById('refAudioInput').addEventListener('input', (e) => { state.refAudioUrl = e.target.value.trim(); });
